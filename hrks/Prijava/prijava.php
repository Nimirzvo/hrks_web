...
elseif($_POST['Prijava_3']){
	...
	$predajeB=$_POST["radio4B"]; // da li osoba izlaže predavanje
	...
	elseif(!empty($autor) && empty($rad)){
	 	$error="Niste upisali naziv priopæenja/referata kao autor.";
		$OK=false;		 
	 }
	 elseif(!empty($koautor) && empty($radKo) && $idReferat==0){
	 	$error="Niste upisali naziv priopæenja/referata kao koautor.";
		$OK=false;		 
	 }
	 if($autor){
		 if(empty($rad)){
	 ...
	 if($OK){
		 ...
		 if($refAutori==1){
			 ...
			 if(!empty($autor)){
				 ...
				 // odredi_referat_ID
				 //echo "prije odreðivanja ID referata <br>";
				 $idRef=odredi_referat_ID($rad, $idSkup);
				 // upis autora u bazu
				...
				if(!empty($radB)){
    				...
    				{
    					$error="Oznaka greške: " . $_FILES["fileB"]["error"] . "<br />";
    				}
  					else
    				{
						//echo $nameFile." naziv datoteke <br>";
						if($nameFileB!=""){
    						if ( file_exists("Datoteke/" . $_FILES["fileB"]["name"]))
      						{
      						  $error=$_FILES["fileB"]["name"] . " veæ postoji u bazi. ";
      						}
    						else
      						{
			...
			$primatelj=odredi_mailAdresu(1);  //  1 - znaèi da je tu mail za domaæe skupove
			...
			if($predaje==1)
				$textNarudzbe.="Izlaže: DA <br>";
			else
				$textNarudzbe.="Izlaže: NE <br>";
			if(!empty($radB)){
			...
			$textNarudzbe2="Hvala, zaprimili smo Vašu prijavu.<br>";
