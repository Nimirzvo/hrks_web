	...
	if($vrstaUplate!=2){		
		...
		if(isset($_POST['box'])){
			...
			// odredimo uplatu za osobu i ime i prezime
				...
				$data=odredi_uplatu_ID_skup($idSkup,$value);
				while ($row = mysql_fetch_array($data)){
					$rezultat=$row;
					break;
				}
...
elseif($_POST['NovaUplata']){ // upis nove uplate za sudionika skupa
    if($vrstaUplate!=2)
			$izbor=3;
		else
			$izbor=31;
        ...
        <table width="100%"  border="0" cellpadding="1" cellspacing="1" bordercolor="#000000">
          <tr>
            <td align="left" valign="top"><form name="form1" enctype="multipart/form-data" method="post" action="<?php  echo "$PHP_SELF"; ?>">
                ...
                <tr align="left">
                  <td colspan="5" height="1" class="tekst">Odabran pregled uplata: <strong><?php echo $nazivUplate ?></strong></td>
                </tr>
                <tr align="left">
                  <td colspan="5" height="1" class="tekst">Ukupno financijsko stanje: <strong><?php echo $nazivUplate ?></strong></td>
                </tr>
                <tr align="left">
                  <td colspan="5" height="1" class="linija">&nbsp;</td>
                </tr>
