	...
	if(isset($_POST['box'])){
		foreach($_POST['box'] as $value){ 
			...
			//echo "SKUP: $idSkup   ID: $value <br>";
   			$file_path = odredi_NazivRada_D($idSkup, $value); 
...
elseif($_POST['Da']){
	...
	while ($row = mysql_fetch_array($rezultat)) {
		$idR=$row[0];
		//echo $idSkup." - skup ".$idR." - id osobe<br>";
		//delete_prijava_E($idSkup, $row[0]); // bisanje
		delete_referati($idSkup, $idR);
	}
}
elseif($_POST['Ne']){
	$izbor=1;
...
elseif($_POST['Prijave']){
		header("Location: skupovi_pregled.php?idSkup=$idSkup"); 
		exit();
}
